...
Severity and urgency are ranked High → Medium → Low.

1) **Blocking Anthropic call inside async loop** (High)
   ...
   - **Success metric:** Long-running Anthropic calls should no longer block `asyncio` tasks, and cancellation signals should be honored within the timeout window.

2) **Truncation assumes paired turns** (Medium)
   ...
   - **Success metric:** Token accounting matches the final retained history, and truncation removes the minimal number of messages needed to fit the budget.

3) **Sparse tool error telemetry** (Medium)
   ...
   - **Success metric:** Tool errors are correlated with traceable types and stack traces, and retry heuristics can differentiate transient from fatal failures.
