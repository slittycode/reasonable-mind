...
def test_architecture(
    layers: int, logic_weight: float, query: str, options: list
) -> dict:
    ...
    print(
        f"🧮 Logic Layers: {result.get('meta_analysis', {}).get('num_logic_layers', 'N/A')}"
    )
    print(
        f"📊 Logic Agreement: {result.get('meta_analysis', {}).get('logic_agreement', 'N/A')}"
    )
...
def demo_scalability_comparison():
    ...
    print("=" * 70)
    print(
        f"{'Arch':<8} {'Time':<10} {'Logic%':<10} {'Logic Layers':<15} {'Confidence':<12} {'Cost/Benefit'}"
    )
    print("-" * 70)
    ...
    for r in results:
        ...
        conf_increase = (
            (r["confidence"] - results[0]["confidence"]) / results[0]["confidence"]
        ) * 100
...
def demo_logic_priority():
    ...
    for logic_weight in [0.50, 0.75, 0.90]:
        ...
        print(
            f"  Logic Agreement: {result.get('meta_analysis', {}).get('logic_agreement', 0):.1%}"
        )
...
def demo_performance_benchmarks():
    ...
    for layers, logic_weight, name in configs:
        tool = ExtendedThinkingTool(
            layers=layers, logic_weight=logic_weight, verbose=False
        )
...
def demo_real_world_use_cases():
    ...
    for uc in use_cases:
        print(f"\n📌 {uc['name']}")
        print(
            f"   Config: {uc['layers']}x layers, {uc['logic_weight']:.0%} logic weight"
        )
