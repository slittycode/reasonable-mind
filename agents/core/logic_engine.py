...
class LogicEngine:
    ...
    def _matches_modus_ponens(self, premises: List[str], conclusion: str) -> bool:
        ...
        has_conditional = any(
            "if" in p.lower() and "then" in p.lower() for p in premises
        )
        has_antecedent = len(premises) >= 2
        conclusion_not_negated = (
            "not" not in conclusion.lower() and "no" not in conclusion.lower()
        )
    ...
    def _matches_modus_tollens(self, premises: List[str], conclusion: str) -> bool:
        ...
        premise_has_negation = any(
            ("not" in p.lower() or "no" in p.lower()) and "if" not in p.lower()
            for p in premises
        )
    ...
    def _matches_hypothetical_syllogism(
        ...
        conditionals = [
            p for p in premises if "if" in p.lower() and "then" in p.lower()
        ]
        has_chain = len(conditionals) >= 2
        conclusion_conditional = (
            "if" in conclusion.lower() and "then" in conclusion.lower()
        )
        return has_chain and conclusion_conditional

    def _matches_disjunctive_syllogism(
        self, premises: List[str], conclusion: str
    ) -> bool:
    ...
    def _matches_affirming_consequent(
        self, premises: List[str], conclusion: str
    ) -> bool:
