...
class ConfidenceCalibrator:
    ...
    def __init__(self, window_size: int = 1000):
        ...
        self.bins: Dict[int, Dict[str, float]] = {
            i: {"total": 0, "correct": 0} for i in range(10)
        }

    def record(
        self, predicted: float, actual_correct: bool, domain: Optional[str] = None
    ) -> None:
    ...
    def _recalculate_bins(self) -> None:
        ...
        self.bins = {i: {"total": 0, "correct": 0} for i in range(10)}
...
class UncertaintyEstimator:
    ...
    def estimate_from_logprobs(
        self, token_logprobs: List[float], key_token_indices: Optional[List[int]] = None
    ) -> ConfidenceEstimate:
        ...
        if key_token_indices:
            relevant_probs = [
                token_logprobs[i] for i in key_token_indices if i < len(token_logprobs)
            ]
        else:
    ...
    def estimate_from_ensemble(
        self, responses: List[str], confidences: List[float]
    ) -> ConfidenceEstimate:
        ...
        conf_std = (
            math.sqrt(sum((c - avg_conf) ** 2 for c in confidences) / n) if n > 1 else 0
        )
    ...
    def combine_estimates(
        self, estimates: List[ConfidenceEstimate], weights: Optional[List[float]] = None
    ) -> ConfidenceEstimate:
        ...
        return ConfidenceEstimate(
            ...
            factors={e.source.value: e.value for e in estimates},
        )
...
class AbstentionPolicy:
    ...
    def _generate_low_confidence_response(self, confidence: ConfidenceEstimate) -> str:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "abstention_rate": self.abstention_count / self.total_queries
...
class UncertaintySystem:
    ...
    def __init__(
        self, confidence_threshold: float = 0.4, calibration_window: int = 1000
    ):
    ...
    def assess(
        ...
        has_hedging = any(
            h in response.lower()
            for h in ["might", "perhaps", "possibly", "not sure", "uncertain"]
        )
        has_citations = "[" in response or "according to" in response.lower()

        estimates.append(
            self.estimator.estimate_from_self_assessment(
                self_reported_confidence, len(response), has_hedging, has_citations
            )
        )
        ...
        if ensemble_responses and ensemble_confidences:
            estimates.append(
                self.estimator.estimate_from_ensemble(
    ...
    def _generate_recommendation(
        self, confidence: ConfidenceEstimate, abstention: AbstentionDecision
    ) -> str:
